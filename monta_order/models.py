    ...
    def clean(self) -> None:
        ...
        if self.pk:
            ...
            if self.stops.filter(status=StatusChoices.COMPLETED):
                raise ValidationError(
                    _("You can't put movement status available if the movement stops are completed.")
                )
...
class Stop(TimeStampedModel):
    ...
    def clean(self) -> None:
        ...
        # if self.movement.status == StatusChoices.IN_PROGRESS or self.movement.status == StatusChoices.COMPLETED:
        #     raise ValidationError(
        #         _("Cannot add new stop if the movement status is in progress or completed")
        #     )
        # if self.movement.order.status == StatusChoices.COMPLETED:
        #     raise ValidationError(
